...
//accounts.insert([{name:'nome',email:'email',mobile:'mobile',start_date:'start_date'}], {w:1}, function(err, result) {});
...
cursor.each(function(err, item) {
	...
	if(item == null) {
		...
		//console.log(stringa);
		stringa += + '["' + 1 + '","' + item.name + '","' + item.email + '","' + item.mobile + '","' + item.start_date + '"],';
	}
});
...
server.get('/', function (req, res) {
	...
	//res.send(JSON.stringify(stringa));
